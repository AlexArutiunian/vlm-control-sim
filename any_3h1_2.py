...
UPPER_HOLD_ZERO = True  # по умолчанию держим нули

# распознавание, какие суставы относятся к верхней части
import re as _re
...
def _is_upper_joint(name: str) -> bool:
    nm = name.lower()
    nm = _re.sub(r"^r\d+_", "", nm)        # срезать r1_/r2_/r3_
    if nm.endswith("_joint"): nm = nm[:-6]
    if any(tok in nm for tok in _LEG_TOKENS):   # быстро отсекаем ноги
        return False
...
# индексы/коэфы PD по верхним суставам
...
ARM_I_CLAMP = 0.7  # интеграл ограничим
...
def setup_arm_pd(zero_pose: bool = True):
    ...
    # соберём все hinge-суставы верха
    ...
    # fallback карта: joint_id -> actuator_id (для actuator с target==этот joint)
...
def _arm_apply_frame(frame_items: list[dict], duration: float, fps: int = 60):
    ...
    # 3) Линеаризация по времени с тиками симуляции
    for k in range(n_steps):
        t = smoothstep((k + 1) / n_steps)
        ARM_TARGETS[:] = start * (1.0 - t) + goal * t

        # не даём "фризу" включиться, пока идёт сценарная анимация
...
def parse_robot_input(sel) -> list[str]:
    ...
    Поддерживает:
      - None / ""           -> [ACTIVE]
      - "all", "*", "both"  -> все доступные из R
      - "r1,r3" / "1,3"     -> ["r1_","r3_"]
      - ["r1_","r3_"]       -> как есть, с фильтром по R
      - "r1_" / "r2" / 2    -> одиночные значения
    Возвращает список нормализованных префиксов, присутствующих в R.
    """
    # 1) Пустое → ACTIVE
    if sel is None or sel == "":
        return [ACTIVE]

    # 2) Наборы: list/tuple/set → склеиваем результат
    if isinstance(sel, (list, tuple, set)):
        out = []
        for item in sel:
            out.extend(parse_robot_input(item))
        # уникализируем в порядке появления и фильтруем по R
        uniq = []
        for p in out:
    ...
    for raw in s.replace(";", ",").split(","):
        ...
        if tok.isdigit():
            ...
            # если уже "r1_" — оставим как есть
            if not tok.endswith("_"):
                # если "r1" без подчёркивания — добавим
    ...
    for t in tokens:
        if t in R and t not in out:
            out.append(t)
    # если ничего не распознали — пусть решает вызывающая сторона через "or [ACTIVE]"
    return out
...
BUSY_SCRIPT = 0  # >0 — идёт сценарная анимация/команда

def _is_idle() -> bool:
    # 0) если двигаем камеру — мы не idle
    if _camera_active():
        return False
    # НОВОЕ: если есть сценарная активность — не idle
    if BUSY_SCRIPT > 0:
        return False
...
# где-нибудь рядом с остальными глобалами
is_dragging = False  # чтобы безопасно использовать в проверках до main()

def _camera_active() -> bool:
    # есть ли какое-нибудь нажатое движение камеры
    return any(movement.get(k, False) for k in ("forward","backward","left","right","rise","fall")) or is_dragging


def _maybe_freeze() -> bool:
    global last_activity_time
    # если двигаем камеру — это активность: не фризим
    if _camera_active():
    ...
    # хотя физику не шагаем — камеру можно обновить
    _update_camera()
...
def do_reset():
    ...
    if ARM_TARGETS is not None:
        if UPPER_HOLD_ZERO:
            ...
            # держать текущую позу после ресета (spawn)
...
def _parse_walk_params(obj: dict) -> tuple[int, float, float]:
    """
    Возвращает (num_steps, dir_deg, speed_mps), поддерживая синонимы:
      steps/num/count, dir_deg/dir/deg/angle/yaw_deg, spd/speed/vel/v
    По умолчанию: dir_deg=0.0, speed=0.5
    Также понимает direction={'forward','back','left','right'}.
    """
    # шаги
    num = obj.get("num")
    if num is None:
    ...
    try:
        ...
        for item in joints_list:
            ...
            if isinstance(item, dict) and "frame" in item:
                ...
                # 4.1 верх — через цели PD
                if upper_list:
                    _arm_apply_frame(upper_list, duration, fps=fps)

                # 4.2 «не верх» — старая логика прямой записи qpos
                if other_list:
                    target_angles = {}
                    for j in other_list:
                        jname = j["name"]
...
def _one_tick(render=True):
    ...
    for p in robots:
        ...
        if task and task['type'] == 'pose':
            ...
            d.xfrc_applied[:] = 0.0
            if now >= t1:
                _update_hold_from_current(p)   # запомнить фактическую конечную позу верха
                ACTIVE_TASK[p] = None
    ...
    if HOLD_UPPER_ZERO:
        for p in robots:
            ...
            if qpos_idx is not None and qpos_idx.size:
                # держим последнюю выставленную позу, а не 0
    ...
    # 2) PD joints
    # 2) PD joints (НОГИ/корпус по политике)
    for p in robots:
        mp = R[p]
    ...
    # === PD контроллер ВЕРХА — ОДИН РАЗ ЗА ТИК (вне цикла по p) ===
...
def key_callback(window_, key, scancode, action, mods):
    ...
    # 'O' — example placeholder: do nothing (empty list)
        ...
        # Обновим ACTIVE, если выбран ровно один
        if len(sel) == 1:
    ...
    # 'R' — hard reset
        ...
        sel = ask_robots(prompt_default_active=True)  # выбор роботов сначала
        cmd_list = open_paste_window()
        if not cmd_list:
            return

        # Если внутри нет указаний "robot"/"robots", обернём в параллель на выбранные
...
def _update_camera(dt=1/2):
    # скорость можно взять из camera_speed, чтобы не дублировать число
    v = camera_speed if 'camera_speed' in globals() else 0.8

    a = np.deg2rad(cam.azimuth)

    # корректные базисные векторы в плоскости XY
    fwd   = np.array([np.cos(a), np.sin(a), 0.0])       # куда «смотрит» камера по азимуту
    right = np.array([np.sin(a), -np.cos(a), 0.0])      # направо от направления взгляда
